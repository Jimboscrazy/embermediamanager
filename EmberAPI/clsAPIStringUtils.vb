...
Public Class StringUtils
    ...
    Public Shared Function CleanStackingMarkers(ByVal sPath As String, Optional ByVal Asterisk As Boolean = False) As String
        ...
        Dim sReturn As String = Regex.Replace(sPath, AdvancedSettings.GetSetting("DeleteStackMarkers", "[\W_]\s?(cd|dvd|part|dis[ck])([0-9])?"), If(Asterisk, "*", " "), RegexOptions.IgnoreCase).Trim
    ...
    Public Shared Function IsStacked(ByVal sName As String, Optional ByVal VTS As Boolean = False) As Boolean
        ...
        If VTS Then
            bReturn = Regex.IsMatch(sName, AdvancedSettings.GetSetting("CheckStackMarkers", "[\W_]+(cd|dvd|part|dis[ck])([0-9])"), RegexOptions.IgnoreCase) OrElse Regex.IsMatch(sName, "^vts_[0-9]+_[0-9]+", RegexOptions.IgnoreCase)
        Else
            bReturn = Regex.IsMatch(sName, AdvancedSettings.GetSetting("CheckStackMarkers", "[\W_]+(cd|dvd|part|dis[ck])([0-9])"), RegexOptions.IgnoreCase)
        End If
        Return bReturn
    End Function
