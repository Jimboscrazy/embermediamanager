...
Public Class NFO
    ...
    Public Shared Function IsConformingEpNfo(ByVal sPath As String) As Boolean
        Dim testSer As XmlSerializer = New XmlSerializer(GetType(XBMC.EpisodeDetails))
        Dim testEp As New XBMC.EpisodeDetails
                ...
                Using xmlSR As StreamReader = New StreamReader(sPath)
                    ...
                    If rMatches.Count = 1 Then
                        Using xmlRead As StringReader = New StringReader(rMatches(0).Value)
                            testEp = DirectCast(testSer.Deserialize(xmlRead), XBMC.EpisodeDetails)
                            testSer = Nothing
                            testEp = Nothing
                            Return True
                        End Using
        ...
        Try
            If (Path.GetExtension(sPath) = ".nfo" OrElse Path.GetExtension(sPath) = ".info") AndAlso File.Exists(sPath) Then
                Using testSR As StreamReader = New StreamReader(sPath)
                    testSer = New XmlSerializer(GetType(XBMC.Movie))
                    Dim testMovie As XBMC.Movie = DirectCast(testSer.Deserialize(testSR), XBMC.Movie)
                    testMovie = Nothing
                    testSer = Nothing
                End Using
                Return True
    ...
    Public Shared Function IsConformingShowNfo(ByVal sPath As String) As Boolean
        ...
        Try
            If (Path.GetExtension(sPath) = ".nfo" OrElse Path.GetExtension(sPath) = ".info") AndAlso File.Exists(sPath) Then
                Using testSR As StreamReader = New StreamReader(sPath)
                    testSer = New XmlSerializer(GetType(XBMC.TVShow))
                    Dim testShow As XBMC.TVShow = DirectCast(testSer.Deserialize(testSR), XBMC.TVShow)
                    testShow = Nothing
                    testSer = Nothing
                End Using
                Return True
    ...
    Public Shared Function LoadMovieFromNFO(ByVal sPath As String, ByVal isSingle As Boolean) As MediaContainers.Movie
        '//
        ' Deserialze the NFO to pass all the data to a XBMC.Movie
        '\\

        Dim xmlSer As XmlSerializer = Nothing
        Dim xmlMov As New XBMC.Movie

        If Not String.IsNullOrEmpty(sPath) Then
            Try
                If File.Exists(sPath) AndAlso Path.GetExtension(sPath).ToLower = ".nfo" Then
                    Using xmlSR As StreamReader = New StreamReader(sPath)
                        xmlSer = New XmlSerializer(GetType(XBMC.Movie))
                        xmlMov = DirectCast(xmlSer.Deserialize(xmlSR), XBMC.Movie)
                        ...
                        Try
                            If Not String.IsNullOrEmpty(sReturn.Text) Then
                                Using xmlSTR As StringReader = New StringReader(sReturn.Text)
                                    xmlSer = New XmlSerializer(GetType(XBMC.Movie))
                                    xmlMov = DirectCast(xmlSer.Deserialize(xmlSTR), XBMC.Movie)
                                    xmlMov.Genre = Strings.Join(xmlMov.LGenre.ToArray, " / ")
                                    xmlMov.Outline = xmlMov.Outline.Replace(vbCrLf, vbLf).Replace(vbLf, vbCrLf)
                                    xmlMov.Plot = xmlMov.Plot.Replace(vbCrLf, vbLf).Replace(vbLf, vbCrLf)
                                    xmlMov.IMDBID = sReturn.IMDBID
    ...
    Public Shared Function LoadTVEpFromNFO(ByVal sPath As String, ByVal SeasonNumber As Integer, ByVal EpisodeNumber As Integer) As MediaContainers.EpisodeDetails
        Dim xmlSer As XmlSerializer = New XmlSerializer(GetType(XBMC.EpisodeDetails))
        Dim xmlEp As New XBMC.EpisodeDetails
                ...
                If File.Exists(sPath) AndAlso Path.GetExtension(sPath).ToLower = ".nfo" Then
                    ...
                    Using xmlSR As StreamReader = New StreamReader(sPath)
                        ...
                        If rMatches.Count = 1 Then
                            ...
                            Using xmlRead As StringReader = New StringReader(rMatches(0).Value)
                                xmlEp = DirectCast(xmlSer.Deserialize(xmlRead), XBMC.EpisodeDetails)
                                xmlSer = Nothing
                                Return ContertToEpsDetais(xmlEp)
                            End Using
                        ElseIf rMatches.Count > 1 Then
                            For Each xmlReg As Match In rMatches
                                Using xmlRead As StringReader = New StringReader(xmlReg.Value)
    ...
    Public Shared Function LoadTVShowFromNFO(ByVal sPath As String) As MediaContainers.TVShow
        Dim xmlSer As XmlSerializer = Nothing
        Dim xmlShow As New XBMC.TVShow

        If Not String.IsNullOrEmpty(sPath) Then
            Try
                If File.Exists(sPath) AndAlso Path.GetExtension(sPath).ToLower = ".nfo" Then
                    Using xmlSR As StreamReader = New StreamReader(sPath)
                        xmlSer = New XmlSerializer(GetType(XBMC.TVShow))
                        xmlShow = DirectCast(xmlSer.Deserialize(xmlSR), XBMC.TVShow)
        ...
        Return ContertToTVShow(xmlShow)
    End Function
    ...
    Public Shared Sub SaveMovieToNFO(ByRef movieToSave As Structures.DBMovie)
        ...
        Try
            ...
            If Not String.IsNullOrEmpty(movieToSave.Filename) Then
                Dim xmlSer As New XmlSerializer(GetType(XBMC.Movie))
    ...
    Public Shared Sub SaveTVEpToNFO(ByRef tvEpToSave As Structures.DBTV)
        Dim _TVEp As XBMC.EpisodeDetails = ContertToXBMCEpsDetais(tvEpToSave.TVEp)
        Try

            If Not String.IsNullOrEmpty(tvEpToSave.Filename) Then
                Dim xmlSer As New XmlSerializer(GetType(XBMC.EpisodeDetails))

                Dim tPath As String = String.Empty
                Dim doesExist As Boolean = False
                Dim fAtt As New FileAttributes
                Dim EpList As New List(Of XBMC.EpisodeDetails)
                    ...
                    Using SQLCommand As SQLite.SQLiteCommand = Master.DB.CreateCommand
                        ...
                        Using SQLreader As SQLite.SQLiteDataReader = SQLCommand.ExecuteReader
                            While SQLreader.Read
                                '_TVEp = Master.DB.LoadTVEpFromDB(Convert.ToInt64(SQLreader("ID")), False).TVEp
                                _TVEp = ContertToXBMCEpsDetais(Master.DB.LoadTVEpFromDB(Convert.ToInt64(SQLreader("ID")), False).TVEp)
                                EpList.Add(_TVEp)
                            End While
                        End Using
                        ...
                        For Each tvEp As XBMC.EpisodeDetails In EpList.OrderBy(Function(s) s.Season)
    ...
    Public Shared Sub SaveTVShowToNFO(ByRef tvShowToSave As Structures.DBTV)
        '//
        ' Serialize XBMC.TVShow to an NFO
        '\\
        Dim _TVShow As XBMC.TVShow = ContertToXBMCTVShow(tvShowToSave.TVShow)
        Try
            If Not String.IsNullOrEmpty(tvShowToSave.ShowPath) Then
                Dim xmlSer As New XmlSerializer(GetType(XBMC.TVShow))
                ...
                If Not doesExist OrElse (Not CBool(File.GetAttributes(tPath) And FileAttributes.ReadOnly)) Then
                    ...
                    Using xmlSW As New StreamWriter(tPath)
                        ...
                        xmlSer.Serialize(xmlSW, _TVShow)
                    End Using
    ...
    Shared Function ContertToEpsDetais(ByVal sEps As XBMC.EpisodeDetails) As MediaContainers.EpisodeDetails
        Dim dEps As New MediaContainers.EpisodeDetails
        Return dEps
    End Function
    Shared Function ContertToXBMCEpsDetais(ByVal sEps As MediaContainers.EpisodeDetails) As XBMC.EpisodeDetails
        Dim dEps As New XBMC.EpisodeDetails
        Return dEps
    End Function
    Shared Function ContertToTVShow(ByVal xTVShow As XBMC.TVShow) As MediaContainers.TVShow
        Dim mTVShow As New MediaContainers.TVShow
        Return mTVShow
    End Function
    Shared Function ContertToXBMCTVShow(ByVal sTVShow As MediaContainers.TVShow) As XBMC.TVShow
        Dim dTVShow As New XBMC.TVShow
        Return dTVShow
    End Function
#End Region 'Methods
#Region "Nested Types"
