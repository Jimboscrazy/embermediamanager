...
Public Class frmMovieInputSettings
    ...
    Private Sub btnDown_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDown.Click
        Dim order As Integer = ModulesManager.Instance.externalInputModules.FirstOrDefault(Function(p) p.AssemblyName = InputXBMC_Module._AssemblyName).ModuleOrder
        If order < ModulesManager.Instance.externalInputModules.Count - 1 Then
            ModulesManager.Instance.externalInputModules.FirstOrDefault(Function(p) p.ModuleOrder = order + 1).ModuleOrder = order
            ModulesManager.Instance.externalInputModules.FirstOrDefault(Function(p) p.AssemblyName = InputXBMC_Module._AssemblyName).ModuleOrder = order + 1
    ...
    Sub orderChanged()
        Dim order As Integer = ModulesManager.Instance.externalInputModules.FirstOrDefault(Function(p) p.AssemblyName = InputXBMC_Module._AssemblyName).ModuleOrder
        btnDown.Enabled = (order < ModulesManager.Instance.externalInputModules.Count - 1)
